...
namespace CarShop.Logic.Services
{
  public class CarLogic : ICarLogic
  {
    ...
    public IEnumerable<Car> GetExpensiveCar()
    {
      var maxPrice = _carRepository.ReadAll().Max(x => x.Price);

      IEnumerable<Car> result = _carRepository.ReadAll().Where(x => x.Price.Equals(maxPrice));

      return result.ToList();
    }

    public IEnumerable<Car> GetCheapCar()
    {
      var maxPrice = _carRepository.ReadAll().Min(x => x.Price);
