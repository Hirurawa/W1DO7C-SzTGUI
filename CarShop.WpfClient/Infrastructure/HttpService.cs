...
namespace CarShop.WpfClient.Infrastructure
{
  public class HttpService
  {
    ...
    public List<T> GetExpensiveCar<T>(string actionName = null)
    {
      using (var client = new HttpClient())
      {
        InitClient(client);

        var response = client.GetAsync(GetActionName(actionName ?? nameof(GetExpensiveCar))).GetAwaiter().GetResult(); // Block here
        return JsonSerializer.Deserialize< List<T>>(response.Content.ReadAsStringAsync().GetAwaiter().GetResult(), serializerOptions);
      }
    }

    public List<T> GetCheapCar<T>(string actionName = null)
    {
      using (var client = new HttpClient())
      {
        InitClient(client);

        var response = client.GetAsync(GetActionName(actionName ?? nameof(GetCheapCar))).GetAwaiter().GetResult(); // Block here
        return JsonSerializer.Deserialize< List<T>>(response.Content.ReadAsStringAsync().GetAwaiter().GetResult(), serializerOptions);
      }
    }
